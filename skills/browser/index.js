...
export default class BrowserSkill {
  constructor(context) {
    ...
    this.backend = null; // 'PROJECT_BROWSER' or 'puppeteer'
    this.PROJECT_BROWSERPath = null;
    this.puppeteerPath = null;
  }

  async init() {
    // Detect MetaPower
    const mpPath = this.config.METAPOWER_PATH || '/root/PROJECT_BROWSER';
    ...
    if (existsSync(browseJs) && existsSync(chromePath)) {
      this.backend = 'PROJECT_BROWSER';
      this.PROJECT_BROWSERPath = mpPath;
  ...
  async browser_open({ url, mode = 'text' }) {
    ...
    if (this.backend === 'PROJECT_BROWSER') {
      return this._PROJECT_BROWSEROpen(url, mode);
    }
  ...
  async browser_act({ url, steps }) {
    ...
    if (this.backend === 'PROJECT_BROWSER') {
      return this._PROJECT_BROWSERAct(url, stepsArr);
    }
  ...
  async browser_screenshot({ url, fullpage = 'false' }) {
    ...
    if (this.backend === 'PROJECT_BROWSER') {
      return this._PROJECT_BROWSERScreenshot(url, outPath);
    }
  ...
  async _PROJECT_BROWSEROpen(url, mode) {
    const browseJs = join(this.PROJECT_BROWSERPath, 'browse.js');
    const flags = {
      text: '',
      html: '--html',
      status: '--status',
    ...
    const cmd = `cd ${this.PROJECT_BROWSERPath} && node browse.js '${this._escapeShell(url)}' ${flag} --wait 3000 --timeout 30000`;
  ...
  async _PROJECT_BROWSERAct(url, steps) {
    const browseJs = join(this.PROJECT_BROWSERPath, 'browse.js');
    const stepsJson = JSON.stringify(steps).replace(/'/g, "'\\''");
    const urlPart = url ? `'${this._escapeShell(url)}'` : "'about:blank'";
    const cmd = `cd ${this.PROJECT_BROWSERPath} && node browse.js ${urlPart} --script '${stepsJson}' --timeout 30000`;
  ...
  async _PROJECT_BROWSERScreenshot(url, outPath) {
    const cmd = `cd ${this.PROJECT_BROWSERPath} && node browse.js '${this._escapeShell(url)}' --screenshot '${outPath}' --wait 3000 --timeout 30000`;
    try {
