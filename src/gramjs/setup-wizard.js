...
async function stepPersonality() {
  ...
  // IDENTITY.md no longer needed — SOUL.md is single source of truth
  ...
  console.log(`${C.green}✅ Generated: SOUL.md, USER.md, MEMORY.md${C.reset}`);
...
// ══════════════════════════════════════════
// Step 2: Instance Setup (Multi-Instance)
// ══════════════════════════════════════════
async function stepInstance(personalityName) {
  header('Step 2/6: Instance Setup');

  console.log(`${C.yellow}🔗 Multi-instance allows multiple MetaClaw bots to communicate via Redis.${C.reset}`);
  const wantInstance = await ask(`${C.yellow}Enable multi-instance? (y/n):${C.reset}`, 'y');

  if (wantInstance.toLowerCase() !== 'y') {
    console.log(`${C.dim}Skipping instance setup.${C.reset}`);
    return null;
  }

  const defaultId = personalityName.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-');
  const instanceId = await ask(`${C.yellow}📛 Instance ID:${C.reset}`, defaultId);
  const instanceName = await ask(`${C.yellow}📛 Instance display name:${C.reset}`, personalityName);

  console.log(`\n${C.yellow}Redis connection:${C.reset}`);
  console.log('  1. Local (redis://localhost:6379) — default');
  console.log('  2. Remote (Upstash/custom URL)');
  const redisChoice = await ask(`${C.yellow}Choice:${C.reset}`, '1');

  let redisUrl = 'redis://localhost:6379';
  if (redisChoice === '2') {
    redisUrl = await ask(`${C.yellow}🔗 Redis URL:${C.reset}`, 'rediss://default:xxx@xxx.upstash.io:6379');
  }

  const prefix = await ask(`${C.yellow}📦 Channel prefix:${C.reset}`, 'metaclaw');

  console.log(`${C.green}✅ Instance: ${instanceId} (${instanceName}) → ${redisUrl}${C.reset}`);
  return { id: instanceId, name: instanceName, redis: { url: redisUrl, prefix } };
}

// ══════════════════════════════════════════
// Step 2 (original): AI Provider
// ══════════════════════════════════════════
async function stepAIProvider(env) {
  header('Step 3/6: AI Provider');
...
// ══════════════════════════════════════════
// Step 6: Connect Telegram & Start PM2
// ══════════════════════════════════════════
async function stepConnectAndStart(personality, instanceConfig) {
  header('Step 6/6: Connect & Launch');

  const wantConnect = await ask(`${C.yellow}🚀 Connect to Telegram now and start the bot? (y/n):${C.reset}`, 'y');
  if (wantConnect.toLowerCase() !== 'y') {
    const pmName = `metaclaw-${instanceConfig?.id || personality.name.toLowerCase()}`;
    console.log(`${C.dim}Skipping auto-connect.${C.reset}`);
    console.log(`${C.yellow}To start manually:${C.reset}`);
    console.log(`  node src/gramjs/index.js      ${C.dim}(first run — login)${C.reset}`);
    console.log(`  pm2 start src/gramjs/index.js --name ${pmName}${C.reset}`);
    return { started: false };
  }

  const pmName = `metaclaw-${instanceConfig?.id || personality.name.toLowerCase().replace(/[^a-z0-9]/g, '-')}`;

  console.log(`\n${C.cyan}📱 Connecting to Telegram...${C.reset}`);
  console.log(`${C.dim}This will start the login flow (phone → code → 2FA if enabled).${C.reset}`);
  console.log(`${C.dim}After successful login, the bot will stop and restart via PM2.${C.reset}\n`);

  const { execSync, spawnSync } = await import('child_process');

  try {
    // Close wizard's readline BEFORE spawning child — prevents stdin conflict
    rl.close();

    console.log(`${C.yellow}⏳ Starting Telegram login...${C.reset}`);
    console.log(`${C.dim}Press Ctrl+C once you see "✅ MetaClaw GramJS Bridge running"${C.reset}\n`);

    // Use spawnSync with stdio inherit — child gets full control of stdin/stdout
    const result = spawnSync('node', ['src/gramjs/index.js'], {
      cwd: ROOT,
      stdio: 'inherit',
      env: { ...process.env },
    });

    // Check if session file exists (login succeeded)
    const sessionFile = path.join(ROOT, 'data/session.txt');
    if (fs.existsSync(sessionFile)) {
      console.log(`\n${C.green}✅ Telegram login successful!${C.reset}`);

      // Start via PM2
      console.log(`${C.cyan}🚀 Starting via PM2 as "${pmName}"...${C.reset}`);
      try {
        execSync(`pm2 start src/gramjs/index.js --name ${pmName}`, { cwd: ROOT, stdio: 'inherit' });
        execSync(`pm2 save`, { stdio: 'inherit' });
        console.log(`${C.green}✅ Bot running as PM2 process "${pmName}"${C.reset}`);
        return { started: true, pmName };
      } catch (pmErr) {
        console.log(`${C.red}⚠️ PM2 start failed: ${pmErr.message}${C.reset}`);
        console.log(`${C.yellow}Start manually: pm2 start src/gramjs/index.js --name ${pmName}${C.reset}`);
        return { started: false, pmName };
      }
    } else {
      console.log(`${C.red}⚠️ Session file not found — login may not have completed.${C.reset}`);
      console.log(`${C.yellow}Try again: node src/gramjs/index.js${C.reset}`);
      return { started: false };
    }
  } catch (err) {
    console.log(`${C.red}⚠️ Connection failed: ${err.message}${C.reset}`);
    console.log(`${C.yellow}Start manually: node src/gramjs/index.js${C.reset}`);
    return { started: false };
  }
}
...
// Summary
// ══════════════════════════════════════════
function showSummary(personality, ai, telegram, owner, instanceConfig, launchResult) {
  const styleLabels = { '1': 'chill & friendly', '2': 'professional', '3': 'fun & playful', '4': 'custom' };
  ...
  if (instanceConfig) {
    console.log(`  Instance: ${C.cyan}${instanceConfig.name} (${instanceConfig.id})${C.reset} → ${instanceConfig.redis.url}`);
  }

  console.log(`  Setup code: ${C.magenta}${owner.code}${C.reset}`);

  if (launchResult?.started) {
    console.log(`\n${C.green}🚀 Bot is LIVE as "${launchResult.pmName}"!${C.reset}`);
    console.log(`${C.yellow}📱 Send:${C.reset} /start ${owner.code} ${C.dim}to your bot on Telegram${C.reset}`);
    console.log(`${C.yellow}📊 Check:${C.reset} pm2 logs ${launchResult.pmName}`);
  } else {
    const pmName = `metaclaw-${instanceConfig?.id || personality.name.toLowerCase()}`;
    console.log(`\n${C.yellow}🚀 To start:${C.reset} pm2 start src/gramjs/index.js --name ${pmName}`);
...
async function main() {
  ...
  const instanceConfig = await stepInstance(personality.name);
  ...
  // Step 6: Connect Telegram & Start PM2 (closes rl internally before spawn)
  const launchResult = await stepConnectAndStart(personality, instanceConfig);

  // Summary
  showSummary(personality, ai, telegram, owner, instanceConfig, launchResult);
